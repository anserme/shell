...
init_panel() {
    ...
    # 输入数字以选择NodeType
    read -p "请选择NodeType（1: V2ray, 2: Shadowsocks, 3: Trojan, 4: Shadowsocks-Plugin）: " node_type_choice
    ...
    *)
    ...
    # 修改配置文件
    sed -i "s#http://127.0.0.1:667#${api_url}#g" /etc/XrayR/config.yml
    sed -i "s/NodeID: 41/NodeID: ${node_id}/g" /etc/XrayR/config.yml
    sed -i "s/ApiKey: \"123\"/ApiKey: \"${token}\"/g" /etc/XrayR/config.yml
    sed -i "s/NodeType: V2ray/NodeType: ${node_type}/g" /etc/XrayR/config.yml
    sed -i "s/EnableREALITY: true/EnableREALITY: false/g" /etc/XrayR/config.yml
    sed -i "s/CertMode: dns/CertMode: http/g" /etc/XrayR/config.yml
